...
## All that matters is the last 5 years
...
epl9213 <- epl.fulldata[1:8226,] #get data from 1992-93 to 2012-13
...
write_excel_csv(EPLSim_Wt, "EPLSimWt.csv")


EPLSim_Wt <- read.csv("C:/Users/nminh/Desktop/USB copy/EPLSimWt.csv")
EPLSim_Wt <- EPLSim_Wt %>% 
  mutate(SimType = "Putting Weights")

table(EPLSim_Wt$Team, EPLSim_Wt$Rank)/nSim
...
cc <- SimComparison %>% 
  group_by(Team, SimType) %>% 
  summarise(Pct = 100*count(Rank)/(nSim))
