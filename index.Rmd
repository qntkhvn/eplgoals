...
![Data Science Diagram, Hadley Wickham, R for Data Science (https://r4ds.had.co.nz/)](data-science.png)
...
We then utilize the Poisson probabilities for each goal values to calculate the expected number of matches associated with those goal categories.
...
write_excel_csv(SimTable, "SimTable.csv")
```

```{r, echo = FALSE, warning=FALSE, message=FALSE}
SimTable <- read.csv("SimTable.csv")
```
...
```{r, warning=FALSE, message=FALSE}
EPLSim_All <- read.csv("EPLSimFull.csv")
EPLSim_All <- EPLSim_All %>% 
...
```{r, message=FALSE}
EPLSim_2010s <- read.csv("EPLSim2010s.csv")
...
```{r}
EPLSim_Wt <- read.csv("EPLSimWt.csv")
EPLSim_Wt <- EPLSim_Wt %>% 
...
read.bibtex(file = "ThesisRef.bib")
```
